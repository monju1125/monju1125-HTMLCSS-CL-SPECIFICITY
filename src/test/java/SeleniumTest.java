...
public class SeleniumTest {
    ...
    private File findHtmlFile() {
        ...
        throw new RuntimeException("Could not find HTML file in any expected location: " + 
    ...
    public void testIDSelectorSpecificity() {
        try {
            ...
            assertEquals("rgba(255, 0, 0, 1)", highSpecificityColor);
            assertEquals("16px", highSpecificityFontSize);
            assertEquals("700", highSpecificityFontWeight);

            // Ensure that the element with lower specificity has the expected properties
            assertEquals("rgba(0, 0, 255, 1)", lowSpecificityColor);
            assertEquals("14px", lowSpecificityFontSize);
            assertEquals("400", lowSpecificityFontWeight);
        } catch (NoSuchElementException e) {
            fail("Elements with IDs not found on the page.");
        }
    }
    ...
    public void testClassSelectorSpecificity() {
        try {
            ...
            // Ensure that the element with higher specificity has the expected properties
            assertEquals("rgba(255, 255, 0, 1)", class2Color);
            assertEquals("20px", class2FontSize);
            assertEquals("700", class2FontWeight);

            // Ensure that the element with lower specificity has the expected properties
            assertEquals("rgba(0, 128, 0, 1)", class1Color);
            assertEquals("18px", class1FontSize);
            assertEquals("400", class1FontWeight);
        } catch (NoSuchElementException e) {
            fail("Elements with classes not found on the page.");
        }
    }
}
